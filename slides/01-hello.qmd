...
# Plan for today {.center background-color="`r red`"}
...
# What's this class about? {.center background-color="`r red`"}
...
# Does **decapitation** work? {.center  background-color="`r red`"}
...
# Does this mean decapitation **causes** terrorist orgs to break down? {.center  background-color="`r red`"}
...
# What is this class about? {.center  background-color="`r red`"}
...
# Let's look at the **website** {.center background-color="`r red`"}
...
# 🚨🚨 Problem sets = 50% 🚨🚨 {.center background-color="`r blue`"}
...
# Try your best and have fun {.center background-color="`r blue`"}





# Who am *I* ? {.center background-color="`r red`"}
...
::: {.column width="50%"}
```{r}
knitr::include_graphics("img/mateo.jpeg")
```
...
# Who are *you* ? {.center background-color="`r red`"}


```{r}
df = read_csv("/Users/juan/Dropbox/teaching/davis/pol51/f23/intake.csv") |> 
  janitor::clean_names()
```
...
df |> 
  ...
  geom_label(aes(label = scales::percent(percent)), fill = blue, color = "white")
```
...
df |> 
  ...
  ggplot(aes(y = fct_reorder(var, percent), x = percent, fill = var == "7")) + 
  geom_col(alpha = .8, color = "white", width = 1) + 
  labs(x = NULL, y = NULL) + 
...
```{r}
df |> 
  select(var = contains("worst_number")) |> 
  mutate(var = as.character(var)) |> 
  tabyl(var) |> 
  slice_max(order_by = n, n = 10) |> 
  ggplot(aes(y = fct_reorder(var, percent), x = percent, fill = var == "7")) + 
...
```{r}
df |> 
  select(var = contains("animal")) |> 
  mutate(var = tolower(var)) |> 
  mutate(var = str_replace(var, " :3", ""),
         var = str_replace(var, "cats", "cat")) |> 
  tabyl(var) |> 
  slice_max(order_by = percent, n = 10) |> 
  ggplot(aes(y = fct_reorder(var, percent), x = percent, fill = var == "cat")) + 
  geom_col(alpha = .8, color = "white", width = 1) + 
  labs(x = NULL, y = NULL) + 
  scale_x_continuous(labels = scales::percent) + 
  scale_fill_manual(values = c(red, blue)) + 
...
## True crime {.scrollable}


```{r}
df |> 
  select(var = contains("crime")) |> 
  sample_n(size = 20) |> 
  kbl(col.names = "Why is true crime so popular?")
```
...
knitr::include_graphics("img/hair-metal.jpeg")
```
...
```{r}
knitr::include_graphics("img/archie.jpeg")
```
...
# UN Voting  {.center background-color="`r red`"}
