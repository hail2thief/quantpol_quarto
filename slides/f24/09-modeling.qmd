...
::: {.column width="50%"}
```{r, out.width="80%"}
knitr::include_graphics(here::here("slides/img/big-picture.jpeg"))
```
:::
...
# OLS in R {.center background-color="`r full`"}
...
The [outcome]{style="color:#dc354a;"} always goes on the **left hand side**

. . .

The [treatment variable]{style="color:#213772;"} goes on the **right hand side**
...
::: {.callout-note}
You'll need to load the `broom` package to use `tidy()` using `library(broom)`
:::
...
# Modeling with continuous variables {.center background-color="`r full`"}
...
::: {.column width="50%"}
```{r}
knitr::include_graphics(here::here("slides/img/dimmer.jpeg"))
```
:::
:::
...
gapminder |> 
  sample_n(4) |> 
  knitr::kable(caption = "Sample from gapminder", digits = 0)
```



- one dollar increase = 0.0007 years increase in life expectancy
- one thousand dollar increase = 0.7 years increase in life expectancy
- ten thousand dollar increase = 7 years increase in life expectancy




## Variable scales {visibility="hidden"}
...
## Re-fit the model {visibility="hidden"}
...
# Regression with categorical variables {.center background-color="`r full`"}
...
::: {.column width="50%"}
```{r}
knitr::include_graphics(here::here("slides/img/switch.jpeg"))
```
:::
:::
